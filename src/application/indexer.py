...
class IndexerService:
    ...
    def run(self):
        ...
        try:
            # 1. Ładowanie dokumentów (DIP: nie wiemy czy to scraper, PDF czy TXT)
            ...
            # 2. Wzbogacanie metadanych o tagi (DIP: nie wiemy jaki to model)
            for chunk in tqdm(chunks, desc="🏷️ Tagowanie"):
                ai_tags = self.tag_generator.generate_tags(chunk.content)
                chunk.metadata["tags"] = list(set(ai_tags))

            # 3. Opcjonalny zapis do JSON (SRP: przeniesione do pomocniczej metody)
    ...
    def _save_checkpoint(self, chunks):
        """Pomocnicza metoda do zapisu stanu (opcjonalna dla SRP)."""
        try:
            ...
            print(f"⚠️ Ostrzeżenie: Nie udało się zapisać kopii JSON: {e}")
