...
class TestIndexerService(unittest.TestCase):
    def setUp(self):
        ...
        self.indexer_service = IndexerService(
            ...
            tag_generator=self.mock_tag_generator,
            processed_dir=self.test_dir
        )
    ...
    def tearDown(self):
        ...
        if os.path.exists(self.test_dir):
            try:
                ...
                print(f"⚠️ Ostrzeżenie: Nie udało się usunąć folderu testowego {self.test_dir} (może być używany).")

    def test_indexer_service_workflow(self):
        # Setup
        chunks = [
            DocumentChunk(content="Art 1 content", metadata={"article": "1", "chapter": "I"})
        ]
